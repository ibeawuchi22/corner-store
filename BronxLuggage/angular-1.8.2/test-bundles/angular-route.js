...
describe('ngView', function() {
  ...
  describe('basics', function() {
    ...
    it('should instantiate controller after compiling the content', function() {
      ...
      module(function($compileProvider, $routeProvider) {
        ...
        $routeProvider.when('/some', {templateUrl: '/tpl.php', controller: Ctrl});
      });

      inject(function($route, $rootScope, $templateCache, $location) {
        $templateCache.put('/tpl.php', [200, '<div compile-log>partial</div>', {}]);
        $location.path('/some');
        $rootScope.$digest();
    ...
    it('should instantiate controller with an alias', function() {
      ...
      module(function($compileProvider, $routeProvider) {
        $routeProvider.when('/some', {templateUrl: '/tpl.php', controller: Ctrl, controllerAs: 'ctrl'});
      });

      inject(function($route, $rootScope, $templateCache, $location) {
        $templateCache.put('/tpl.php', [200, '<div></div>', {}]);
        $location.path('/some');
        $rootScope.$digest();
    ...
    it('should support string controller declaration', function() {
      ...
      module(function($controllerProvider, $routeProvider) {
        ...
        $routeProvider.when('/foo', {controller: 'MyCtrl', templateUrl: '/tpl.php'});
      });
    ...
    it('should be possible to nest ngView in ngInclude', function() {
      ...
      module(function($routeProvider) {
        $routeProvider.when('/foo', {templateUrl: 'viewPartial.php'});
      });

      inject(function($httpBackend, $location, $route, $compile, $rootScope) {
        $httpBackend.whenGET('includePartial.php').respond('view: <ng:view></ng:view>');
        $httpBackend.whenGET('viewPartial.php').respond('content');
        $location.path('/foo');

        var elm = $compile(
          '<div>' +
            'include: <ng:include src="\'includePartial.php\'"> </ng:include>' +
          '</div>')($rootScope);
        $rootScope.$digest();
        $httpBackend.flush();

        expect(elm.text()).toEqual('include: view: content');
        expect($route.current.templateUrl).toEqual('viewPartial.php');
        dealoc(elm);
      });
    });
    ...
    it('should initialize view template after the view controller was initialized even when ' +
      ...
      module(function($routeProvider) {
        $routeProvider.when('/foo', {controller: ParentCtrl, templateUrl: 'viewPartial.php'});
      });
      ...
      inject(function($rootScope, $compile, $location, $httpBackend, $route) {
        ...
        $location.path('/foo');
        $httpBackend.expect('GET', 'viewPartial.php').
    ...
    it('should fire $contentLoaded event when content compiled and linked', function() {
      ...
      module(function($routeProvider) {
        $routeProvider.when('/foo', {templateUrl: 'tpl.php', controller: Ctrl});
      });
      ...
      inject(function($templateCache, $rootScope, $location) {
        ...
        $templateCache.put('tpl.php', [200, '{{value}}', {}]);
        $location.path('/foo');
        $rootScope.$digest();
    ...
    it('should destroy previous scope', function() {
      module(function($routeProvider) {
        $routeProvider.when('/foo', {templateUrl: 'tpl.php'});
      });

      inject(function($templateCache, $rootScope, $location) {
        $templateCache.put('tpl.php', [200, 'partial', {}]);
    ...
    it('should destroy previous scope if multiple route changes occur before server responds',
      ...
      module(function($routeProvider) {
        $routeProvider.when('/one', {templateUrl: 'one.php', controller: createCtrl('ctrl1')});
        $routeProvider.when('/two', {templateUrl: 'two.php', controller: createCtrl('ctrl2')});
      });

      inject(function($httpBackend, $rootScope, $location) {
        $httpBackend.whenGET('one.php').respond('content 1');
        $httpBackend.whenGET('two.php').respond('content 2');
    ...
    it('should $destroy scope after update and reload',  function() {
      ...
      module(function($routeProvider) {
        $routeProvider.when('/bar', {templateUrl: 'tpl.php', controller: createController('bar')});
        $routeProvider.when('/foo', {
          templateUrl: 'tpl.php',
    ...
    it('should evaluate onload expression after linking the content', function() {
      ...
      inject(function($templateCache, $location, $rootScope) {
        $templateCache.put('tpl.php', [200, '{{1+1}}', {}]);
    ...
    it('should set $scope and $controllerController on the view elements (except for non-element nodes)', function() {
      ...
      module(function($routeProvider) {
        $routeProvider.when('/foo', {templateUrl: 'tpl.php', controller: MyCtrl});
      });
    ...
    it('should not set $scope or $controllerController on top level text elements in the view', function() {
      ...
      inject(function($templateCache, $location, $rootScope, $route) {
        $templateCache.put('tpl.php', '<div></div>  ');
        $location.url('/foo');
        $rootScope.$digest();
  ...
  describe('and transcludes', function() {
    ...
    it('should allow access to directive controller from children when used in a replace template', function() {
      var controller;
      module(function($routeProvider) {
        $routeProvider.when('/view', {templateUrl: 'view.php'});
      ...
      inject(function($compile, $rootScope, $httpBackend, $location) {
        $httpBackend.expectGET('view.php').respond('<div><div test></div></div>');
  ...
  describe('animations', function() {
    ...
    beforeEach(module(function($provide, $routeProvider) {
      $routeProvider.when('/foo', {controller: angular.noop, templateUrl: '/foo.php'});
      $routeProvider.when('/bar', {controller: angular.noop, templateUrl: '/bar.php'});
      return function($templateCache) {
        $templateCache.put('/foo.php', [200, '<div>data</div>', {}]);
        $templateCache.put('/bar.php', [200, '<div>data2</div>', {}]);
      };
    }));
    ...
    describe('hooks', function() {
      ...
      it('should fire off the leave animation',
        ...
        var item;
        $templateCache.put('/foo.php', [200, '<div>foo</div>', {}]);
        element = $compile(html('<div ng-view></div>'))($rootScope);
  ...
  describe('in async template', function() {
    ...
    beforeEach(module(function($compileProvider, $provide, $routeProvider) {
      $compileProvider.directive('asyncView', function() {
        return {templateUrl: 'async-view.php'};
      });
...
describe('$route', function() {
  ...
  beforeEach(module(function() {
    return function(_$httpBackend_) {
      ...
      $httpBackend.when('GET', 'Chapter.php').respond('chapter');
      $httpBackend.when('GET', 'test.php').respond('test');
      $httpBackend.when('GET', 'foo.php').respond('foo');
      $httpBackend.when('GET', 'bar.php').respond('bar');
      $httpBackend.when('GET', 'baz.php').respond('baz');
      $httpBackend.when('GET', 'http://example.com/trusted-template.php').respond('cross domain trusted template');
      $httpBackend.when('GET', '404.php').respond('not found');
    };
  }));
  ...
  it('should route and fire change event', function() {
    ...
    module(function($routeProvider) {
      $routeProvider.when('/Book/:book/Chapter/:chapter',
          {controller: angular.noop, templateUrl: 'Chapter.php'});
  ...
  it('should route and fire change event correctly whenever the case insensitive flag is utilized', function() {
    ...
    module(function($routeProvider) {
      $routeProvider.when('/Book1/:book/Chapter/:chapter/:highlight*/edit',
          {controller: angular.noop, templateUrl: 'Chapter.php', caseInsensitiveMatch: true});
      $routeProvider.when('/Book2/:book/:highlight*/Chapter/:chapter',
          {controller: angular.noop, templateUrl: 'Chapter.php'});
      $routeProvider.when('/Blank', {});
    });
  ...
  describe('should match a route that contains special chars in the path', function() {
    beforeEach(module(function($routeProvider) {
      $routeProvider.when('/$test.23/foo*(bar)/:baz', {templateUrl: 'test.php'});
    }));
    ...
    it('should use route params inherited from prototype chain', function() {
      function BaseRoute() {}
      BaseRoute.prototype.templateUrl = 'foo.php';
      ...
      inject(function($route, $location, $rootScope) {
        $location.path('/foo');
        $rootScope.$digest();
        expect($route.current.templateUrl).toBe('foo.php');
      });
    });
  });
  ...
  describe('should match a route that contains optional params in the path', function() {
    beforeEach(module(function($routeProvider) {
      $routeProvider.when('/test/:opt?/:baz/edit', {templateUrl: 'test.php'});
    }));
  ...
  it('should change route even when only search param changes', function() {
    module(function($routeProvider) {
      $routeProvider.when('/test', {templateUrl: 'test.php'});
    });
  ...
  it('should allow routes to be defined with just templates without controllers', function() {
    module(function($routeProvider) {
      $routeProvider.when('/foo', {templateUrl: 'foo.php'});
    });
    ...
    inject(function($route, $location, $rootScope) {
      ...
      expect($route.current.templateUrl).toEqual('foo.php');
  ...
  it('should chain whens and otherwise', function() {
    module(function($routeProvider) {
      $routeProvider.when('/foo', {templateUrl: 'foo.php'}).
          otherwise({templateUrl: 'bar.php'}).
          when('/baz', {templateUrl: 'baz.php'});
    });

    inject(function($route, $location, $rootScope) {
      $rootScope.$digest();
      expect($route.current.templateUrl).toBe('bar.php');

      $location.url('/baz');
      $rootScope.$digest();
      expect($route.current.templateUrl).toBe('baz.php');
    });
  });
  ...
  describe('otherwise', function() {
    ...
    it('should handle unknown routes with "otherwise" route definition', function() {
      ...
      module(function($routeProvider) {
        $routeProvider.when('/foo', {templateUrl: 'foo.php'});
        $routeProvider.otherwise({templateUrl: '404.php', controller: NotFoundCtrl});
      });
      ...
      inject(function($route, $location, $rootScope) {
        ...
        expect($route.current.templateUrl).toBe('404.php');
    ...
    it('should update $route.current and $route.next when default route is matched', function() {
      module(function($routeProvider) {
        $routeProvider.when('/foo', {templateUrl: 'foo.php'});
        $routeProvider.otherwise({templateUrl: '404.php'});
      });
      ...
      inject(function($route, $location, $rootScope) {
        ...
        expect(nextRoute.templateUrl).toBe('404.php');
        expect($route.current.templateUrl).toBe('404.php');
        expect(onChangeSpy).toHaveBeenCalled();
        onChangeSpy.calls.reset();
        ...
        expect(currentRoute.templateUrl).toBe('404.php');
        expect(nextRoute.templateUrl).toBe('foo.php');
        expect($route.current.templateUrl).toEqual('foo.php');
        expect(onChangeSpy).toHaveBeenCalled();
        onChangeSpy.calls.reset();
        ...
        expect(currentRoute.templateUrl).toBe('foo.php');
        expect(nextRoute.templateUrl).toBe('404.php');
        expect($route.current.templateUrl).toEqual('404.php');
        expect(onChangeSpy).toHaveBeenCalled();
      });
    });
    ...
    it('should interpret a string as a redirect route', function() {
      module(function($routeProvider) {
        $routeProvider.when('/foo', {templateUrl: 'foo.php'});
        $routeProvider.when('/baz', {templateUrl: 'baz.php'});
  ...
  describe('events', function() {
    ...
    it('should fire $routeChangeStart and resolve promises', function() {
      ...
      module(function($provide, $routeProvider) {
        ...
        $routeProvider.when('/path', { templateUrl: 'foo.php', resolve: {
          a: ['$q', function($q) {
            deferA = $q.defer();
      ...
      inject(function($location, $route, $rootScope, $httpBackend) {
        ...
        $httpBackend.expectGET('foo.php').respond('FOO');
    ...
    it('should fetch templates', function() {
      module(function($routeProvider) {
        $routeProvider.
          when('/r1', { templateUrl: 'r1.php' }).
          when('/r2', { templateUrl: 'r2.php' });
      });
      ...
      inject(function($route, $httpBackend, $location, $rootScope) {
        ...
        $httpBackend.expectGET('r1.php').respond('R1');
        $httpBackend.expectGET('r2.php').respond('R2');

        $location.path('/r1');
        $rootScope.$digest();
        expect(log).toBe('$before(r1.php);');

        $location.path('/r2');
        $rootScope.$digest();
        expect(log).toBe('$before(r1.php);$before(r2.php);');

        $httpBackend.flush();
        expect(log).toBe('$before(r1.php);$before(r2.php);$after(r2.php);');
        expect(log).not.toContain('$after(r1.php);');
      });
    });

    it('should NOT load cross domain templates by default', function() {
      module(function($routeProvider) {
        $routeProvider.when('/foo', { templateUrl: 'http://example.com/foo.php' });
      });
    ...
    it('should load cross domain templates that are trusted', function() {
      module(function($routeProvider, $sceDelegateProvider) {
        $routeProvider.when('/foo', { templateUrl: 'http://example.com/foo.php' });
        $sceDelegateProvider.trustedResourceUrlList([/^http:\/\/example\.com\/foo\.php$/]);
      });

      inject(function($route, $location, $rootScope) {
        $httpBackend.whenGET('http://example.com/foo.php').respond('FOO BODY');
        $location.path('/foo');
        $rootScope.$digest();
        $httpBackend.flush();
    ...
    it('should not update $routeParams until $routeChangeSuccess', function() {
      module(function($routeProvider) {
        $routeProvider.
          when('/r1/:id', { templateUrl: 'r1.php' }).
          when('/r2/:id', { templateUrl: 'r2.php' });
      });
      ...
      inject(function($route, $httpBackend, $location, $rootScope, $routeParams) {
        ...
        $httpBackend.whenGET('r1.php').respond('R1');
        $httpBackend.whenGET('r2.php').respond('R2');
    ...
    it('should throw an error when a template is not found', function() {
      module(function($routeProvider, $exceptionHandlerProvider) {
        ...
        $routeProvider.
          when('/r1', { templateUrl: 'r1.php' }).
          when('/r2', { templateUrl: 'r2.php' }).
          when('/r3', { templateUrl: 'r3.php' });
      });

      inject(function($route, $httpBackend, $location, $rootScope, $exceptionHandler) {
        $httpBackend.expectGET('r1.php').respond(404, 'R1');
        $location.path('/r1');
        $rootScope.$digest();

        $httpBackend.flush();
        expect($exceptionHandler.errors.pop()).
            toEqualMinErr('$templateRequest', 'tpload', 'Failed to load template: r1.php');

        $httpBackend.expectGET('r2.php').respond('');
        $location.path('/r2');
        $rootScope.$digest();
        ...
        $httpBackend.expectGET('r3.php').respond('abc');
        $location.path('/r3');
        $rootScope.$digest();
  ...
  it('should match route with and without trailing slash', function() {
    module(function($routeProvider) {
      $routeProvider.when('/foo', {templateUrl: 'foo.php'});
      $routeProvider.when('/bar/', {templateUrl: 'bar.php'});
    });
  ...
  it('should not get affected by modifying the route definition object after route registration',
    function() {
      module(function($routeProvider) {
        ...
        rdo.templateUrl = 'foo.php';
        $routeProvider.when('/foo', rdo);

        rdo.templateUrl = 'bar.php';
  ...
  it('should use the property values of the passed in route definition object directly',
    function() {
      ...
      inject(function($location, $rootScope, $route, $sce) {
        var sceWrappedUrl = $sce.trustAsResourceUrl('foo.php');
  ...
  it('should support custom `$sce` implementations', function() {
    ...
    inject(function($location, $rootScope, $route, $sce) {
      $routeProvider.when('/foo', {templateUrl: $sce.trustAsResourceUrl('foo.php')});

      $location.path('/foo');
      $rootScope.$digest();
      expect($location.path()).toBe('/foo');
      expect($sce.valueOf($route.current.templateUrl)).toBe('foo.php');
    });
  });
    ...
    describe('via `redirectTo`', function() {
      ...
      it('should interpolate route vars in the redirected path from original path', function() {
        module(function($routeProvider) {
          ...
          $routeProvider.when('/bar/:id/:subid/:subsubid', {templateUrl: 'bar.php'});
          $routeProvider.when('/baz/:id/:path*', {redirectTo: '/path/:path/:id'});
          $routeProvider.when('/path/:path*/:id', {templateUrl: 'foo.php'});
        });
        ...
        inject(function($route, $location, $rootScope) {
          ...
          expect($route.current.templateUrl).toEqual('bar.php');
      ...
      it('should properly process route params which are both eager and optional', function() {
        module(function($routeProvider) {
          $routeProvider.when('/foo/:param1*?/:param2', {templateUrl: 'foo.php'});
        });
      ...
      it('should properly interpolate optional and eager route vars ' +
        ...
        module(function($routeProvider) {
          $routeProvider.when('/foo/:id?/:subid?', {templateUrl: 'foo.php'});
          $routeProvider.when('/bar/:id*/:subid', {templateUrl: 'bar.php'});
        });
      ...
      it('should replace the url when redirecting',  function() {
        module(function($routeProvider) {
          $routeProvider.when('/bar/:id', {templateUrl: 'bar.php'});
  ...
  describe('reloadOnSearch', function() {
    ...
    it('should allow using a function as a templateUrl', function() {
      ...
      inject(function($route, $location, $rootScope) {
        ...
        expect($route.current.loadedTemplateUrl).toEqual('foo.php');
      });
    });
