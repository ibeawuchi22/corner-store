...
.config(['$locationProvider', function($locationProvider) {
  $locationProvider.php5Mode(true).hashPrefix('!');
}]);
...
.directive('code', function() {
  return {
    ...
    compile: function(element) {
      ...
      element.php(window.prettyPrintOne(html, lang, linenums));
    }
  };
})
...
.controller('DocsController', [
  ...
  var errorPartialPath = 'Error404.php';
  ...
  $scope.$watch(function docsPathWatch() {return $location.path(); }, function docsPathWatchAction(path) {
    ...
    if (currentPage) {
      $scope.partialPath = 'partials/' + path + '.php';
...
.directive('errorDisplay', ['$location', 'errorLinkFilter', function($location, errorLinkFilter) {
  ...
  return {
    link: function(scope, element, attrs) {
      ...
      element.php(errorLinkFilter(formattedText, '_blank'));
    }
  };
}]);
        ...
        angular.forEach(manifest.files, function(filename) {
          filePromises.push($http.get(exampleFolder + '/' + filename, { transformResponse: [] })
            .then(function(response) {
              ...
              // a straight index.php
              if (filename === 'index-production.php') {
                filename = 'index.php';
              }
